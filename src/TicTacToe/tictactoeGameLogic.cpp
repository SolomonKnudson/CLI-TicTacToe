...
TicTacToe::_gameLoop()
{
  while (!m_hasWon && !m_tie)
  {
    ...
    if (!_isLegalMove())
    {
      ...
      if (_currentPlayer().invalidMoves() > m_totalInvalidMovesAllowed)
      {
...
// as debug commands do not add to player moves. if in debug, skip check.
    ...
    if (_currentPlayer().moves() >=
        ((m_previousRow < m_previousColumn) ? m_previousRow : m_previousColumn))
    {
      if (_isWinningMove())
      {
        m_hasWon = true;
...
TicTacToe::_endGame()
{
  if (m_hasWon)
  {
    std::cout << "|Tic-Tac-Toe!\n|Congrats, " << _currentPlayer().name()
              << "! You Win!\n";

    std::cout << "|You beat "
              << ((m_currentPlayerMark == m_playerX.mark()) ? m_playerO.name()
                                                            : m_playerX.name())
              << " in " << _currentPlayer().moves() << " moves!";
...
TicTacToe::_resetGame()
{
  ...
  else if (m_tie)
  {
    ...
    std::cout << "|Last game was a tie; " << _currentPlayer().name()
              << " will start!\n";
  }
...
TicTacToe::_quit() const
{
  ...
  {
    std::cout << "\n|Congrats, "
              << ((m_playerX.wins() > m_playerO.wins()) ? m_playerX.name()
                                                        : m_playerO.name())
              << "!\n|You had the most wins with: "
              << ((m_playerX.wins() > m_playerO.wins()) ? m_playerX.wins()
                                                        : m_playerO.wins())
              << "!\n";
  }
  else
